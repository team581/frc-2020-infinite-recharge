...
public class ToggleSnarferIntakeCommand extends InstantCommand {
  ...
  public void initialize() {
    ...
    if (RobotContainer.snarferSubsystem.preferredDirection == SnarferIntakeDirection.IN) {
      RobotContainer.snarferSubsystem.preferredDirection = SnarferIntakeDirection.STOPPED;
    } else {
      // The snarfer is not grabbing power cells, so start the intake
      RobotContainer.snarferSubsystem.preferredDirection = SnarferIntakeDirection.IN;
    }
  }
}
